...
data = pd.read_excel('exp2GraficasLocGlobal.xlsx')
...
def create_plot(ilum, ilum1, ilum2, ilum4, ilum8):
    ...
    # plt.suptitle('Global localization', fontsize=24)
    # plt.title('Illumination condition: ' + ilum, fontsize=20)
    # plt.title(f'Experiment 1, Global localization, Illumination: {ilum}', fontsize=18)
    plt.title(f'Experiment 1, Global localization', fontsize=18)
    plt.legend(fontsize=14)
    plt.grid()
    plt.savefig("exp1RecallGlobalLoc" + ilum + ".png")
    return
