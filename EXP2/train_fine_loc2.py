...
with open(csvDir + "/Exp2TrainingDataFineLoc.csv", 'w', newline='') as file:
    ...
    for lossFunction in selectedLosses:
        ...
        lossDir_Lg = create_path(os.path.join(baseDir, "SAVED_MODELS", "EXP2", "CoarseLoc", "BestNets", sl))
        lossDir_Lf = create_path(os.path.join(baseDir, "SAVED_MODELS", "EXP2", "FineLoc", sl))
        ...
        for room in rooms:
            ...
            vmDataset = create_datasets2.VisualModelTrainFineLoc(
            ...
            trainDataset = create_datasets2.TrainFineLoc(
                tree=treeCoordsVM, currentRoom=room, imageFolderDataset=dset.ImageFolder(datasetDir + "/Train/"))
            trainDataloader = DataLoader(trainDataset, shuffle=True, num_workers=0,
                                         batch_size=16)

            valDataset = create_datasets2.ValidationFineLoc(
                currentRoom=room, imageFolderDataset=os.path.join(datasetDir, "AugmentedSets", "Validation", room))
            ...
            for epoch in range(0, PARAMETERS.numEpochsFineLoc):
                ...
                for i, data in enumerate(trainDataloader, 0):
                    ...
                    if i % PARAMETERS.doValidation == 0:
                        recallk1, geomError, minErrorPossible = 0, 0, 0
                        ...
                        recallk1 *= 100 / len(valDataloader)

                        print(f"Average recall (k=1)= {recallk1} %")
                        ...
                        if geomError <= bestError:
                            ...
                            if geomError <= 0.30:
                                ...
                                print(f"Validation recall: {recallk1} %, Geometric error: {geomError} m\n\n")

                                writer.writerow([lossFunction, margin, room, epoch, i, recallk1, bestError])
                ...
                if epochs_since_improvement == early_stopping_threshold or recallk1 >= 100:
